...
import type { ChatMessage, ChatSummary } from '@/types/messages';
    ...
    (set, get) => ({
      ...
      loadChats: async () => {
        ...
        try {
          ...
          console.error('[messages-store] 加载私信列表失败:', error);
      ...
      loadMoreMessages: async (chatId: string) => {
        ...
        try {
          ...
          console.error('[messages-store] 加载历史消息失败:', error);
        } finally {
          set(s => ({
      ...
      sendMessage: async (chatId, content) => {
        try {
          ...
          console.error('[messages-store] 消息发送失败:', error);
          throw error;
        }
      },
      ...
      clearMessages: async (chatId: string) => {
        try {
          ...
          console.error('[messages-store] 清空消息失败:', error);
          throw error;
        }
      },
