...
import { FriendRequestsDrawer } from '@/components/messages/friend-requests-drawer';
...
import type { FriendItem } from '@/stores/contacts-store';
...
export default function MessagesCenterPage() {
  ...
  const handleSelectFriend = useCallback(
    async (friend: FriendEntry) => {
      ...
      // AI 助手使用虚拟 chatId，不需要创建真实私信会话
      if (friend.id === 'ai-assistant') {
        setSelectedChatId('ai-chat');
        return;
      }

      // 如果好友已经有 chatId，直接使用；否则创建新的私聊会话
      if (friend.chatId) {
  ...
  useEffect(() => {
    if (!selectedFriend && allFriends.length > 0) {
      const requestedFriend = requestedChatId
        ? allFriends.find(friend => friend.chatId === requestedChatId)
        : null;
      const preferredFriend = shouldOpenAssistant
        ? (allFriends.find(friend => friend.id === 'ai-assistant') ?? null)
        : null;
      const firstFriend = requestedFriend ?? preferredFriend ?? allFriends[0];
      ...
      <Dialog open={clearConfirmOpen} onOpenChange={setClearConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              此操作将清除当前私信会话中的所有消息记录（仅对你可见），对方不会受到影响，且无法恢复。确定要继续吗？
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
